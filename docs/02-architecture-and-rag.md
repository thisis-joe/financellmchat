...
```text
질문에서 나이와 목적 감지
-> 청년/장병/아동 등 부적합 상품 제외
-> 목적 힌트가 있으면 관련 상품 우선
-> 추천 후보 3개와 이유 설명
-> 각 후보의 원본 PDF 출처 제공
```

예를 들어 `50대인데 추천해주세요`라고 물으면 청년도약계좌, 청년 주택드림 청약통장, 장병내일준비적금처럼 특정 연령이나 군복무 조건이 강한 상품은 우선 제외한다. 대신 `BNK내맘대로 적금`, `백세청춘 실버적금`, `Only One 주거래 우대적금`처럼 확인해 볼 만한 후보를 보여준다.
