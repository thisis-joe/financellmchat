...
답변 이후에는 `historyId`가 중요해진다. Spring은 한 번의 질문과 답변을 `query_histories`에 저장하고, 같은 이력에 대해 피드백과 검색 근거 조회를 연결한다. 그래서 프론트는 답변을 받은 뒤 작은 `답변근거` 버튼으로 `GET /api/histories/{historyId}/evidence`를 호출할 수 있다.
...
`build_direct_response`는 검색 전에 인사말, 예금 종류 요청, 추천 요청을 먼저 처리한다. `ㅎㅇ` 같은 입력은 문서 검색이 아니라 인사 응답으로 끝내고, `예금 종류`는 예금상품 분류를 안내한다. `50대 추천` 같은 요청은 추천 전용 흐름에서 나이와 목적을 보고 후보를 고른다.

`load_documents`는 MySQL에서 부산은행 적립식예금 문서 chunk를 읽는다. 현재 기본 검색 범위는 `예금상품>적립식예금`이다. `retrieve_documents`는 질문에 맞는 chunk를 찾는다. `generate_answer`는 검색 근거를 바탕으로 Gemma LLM 답변을 만들고, 실패하면 문서기반 추출 답변으로 돌아간다.

LangGraph는 이 단계를 그래프로 연결한다. 여기서 LangGraph는 자율 에이전트라기보다 RAG 상태가 단계별로 어떻게 바뀌는지 명확히 보여주는 워크플로 도구다.

질문 유형 분류를 둔 이유는 단순하다. 모든 입력을 RAG 검색으로 보내면 인사말도 청년 상품 문서로 매칭될 수 있고, `예금 종류`도 약관 문서로 흘러갈 수 있다. RAG는 문서 근거가 필요한 질문에만 쓰고, 목록 안내나 인사말은 더 단순한 경로로 처리하는 편이 정확하다.
...
상품명이 질문에 명시된 경우에는 다른 상품 문서가 출처에 섞이지 않도록 상품 불일치 패널티를 둔다. 또 `50대 추천`처럼 나이가 들어간 질문에서는 숫자 `50`이 `50만원` 같은 금액 표현과 섞이지 않도록 나이 표현을 별도로 감지한다.

## 추천 응답

추천은 일반 RAG 검색과 분리한다. 추천 질문은 단일 정답을 찾는 검색 문제가 아니라 조건에 맞지 않는 상품을 제외하고 후보를 설명하는 문제이기 때문이다.
...
예를 들어 `50대인데 추천해주세요`라고 물으면 청년도약계좌, 청년 주택드림 청약통장, 장병내일준비적금처럼 특정 연령이나 군복무 조건이 강한 상품은 우선 제외한다. `50대`처럼 나이대만 말한 경우에는 백세청춘 실버적금도 만 56세 이상 조건을 모든 50대가 만족하는 것은 아니므로 제외 사유에 표시한다. 대신 `BNK내맘대로 적금`, `Only One 주거래 우대적금`, `정기적금`처럼 PDF 가입대상상 더 넓게 확인 가능한 후보를 보여준다.

이후 `10대인데 추천해주세요` 문제를 보면서 추천 필터를 한 단계 더 구체화했다. 상품명에 `청년`이 들어가는지만 보는 것이 아니라, PDF 본문의 `가입대상`, `가입자격`, `가입제한` 구간에서 `만 19세 이상`, `만 34세 이하`, `연령에 관계없이`, `제한없음` 같은 조건을 읽고 후보를 걸러낸다. 그래서 10대 또는 만 18세 질문에는 청년도약계좌처럼 PDF상 만 19세 이상 조건이 있는 상품을 추천 출처에서 제외한다.

이 추천은 금융 자문이 아니라 상품공시 기반 후보 안내다. 그래서 답변 끝에는 실제 가입 가능 여부와 우대금리가 가입 시점, 개인 조건, 은행 확인 결과에 따라 달라질 수 있다고 안내한다.

현재 한계도 분명하다. 추천 검증은 PDF 텍스트에서 규칙으로 읽을 수 있는 나이 조건을 우선 처리한다. 하지만 청년도약계좌의 소득 조건, 청년 주택드림 청약통장의 무주택 조건, 부산청년기쁨두배통장의 부산시 선정 조건처럼 여러 조건이 결합된 자격은 아직 완전 자동 판정하지 않는다. 이런 조건은 답변에서 추가 확인 필요로 남기고, 다음 단계에서 조건 추출 범위를 넓히는 것이 맞다.

또한 현재 PDF 처리 기반은 텍스트 레이어 추출이다. 표가 이미지로 들어간 PDF나 OCR이 필요한 표 안의 글자는 검색과 조건 검증에서 누락될 수 있다. 따라서 지금의 가입대상 검증은 “PDF 내용을 확인하려는 방향으로 개선한 1단계”이지, 은행 창구 수준의 최종 가입 가능 판정은 아니다.
...
`RagService.evidence`는 새 검색을 다시 돌리지 않는다. 이미 답변 당시 저장한 `citationsJson`을 읽어 순위, 제목, score, snippet을 보여준다. 이 방식은 사용자가 본 답변과 같은 시점의 근거를 확인하게 해 주므로 디버깅에 더 적합하다.
...
학습 데이터셋 포맷은 `10-lora-qlora-dataset.md`에 따로 정리한다. 핵심은 `query_histories`의 질문, 답변, `citationsJson`과 `answer_feedbacks`의 평가를 연결해 JSONL chat messages 형태로 만드는 것이다.
...
- 질문 유형 분류: 사용자의 입력을 검색 전에 인사말, 목록 요청, 추천 요청, 일반 문서 검색으로 나누는 처리다.
- 상품 불일치 패널티: 질문에 특정 상품명이 있을 때 다른 상품 문서의 검색 점수를 낮추는 보정이다.
- 추천 흐름: 검색 점수만으로 답하지 않고 조건에 맞는 후보를 고르고 제외 이유를 함께 설명하는 처리다.
- 답변근거 API: 저장된 질문 이력의 검색 근거를 다시 보여주는 디버그 API다.
- 가입대상 검증: 추천 후보를 정하기 전에 PDF의 가입 조건 구간을 읽고 나이 조건과 맞지 않는 상품을 제외하는 처리다.
