...
이후 `bnkaichat.xyz` 접속에서 Cloudflare `1016` 오류도 확인했다. 처음에는 HTTPS 인증서 문제처럼 보였지만, 실제로는 Cloudflare가 프론트 도메인의 원본 서버를 DNS로 찾지 못하는 문제였다. 그래서 인증서를 직접 발급하는 방향이 아니라, Vercel 프로젝트에 `bnkaichat.xyz`를 추가하고 Cloudflare DNS의 `@`, `www` 레코드를 Vercel로 보내며 프록시는 `DNS only`로 두는 방향으로 정리했다. 이렇게 해야 Vercel이 커스텀 도메인을 검증하고 HTTPS 인증서를 자동 발급할 수 있다.
...
- DNS only: Cloudflare가 DNS 응답만 하고 HTTP/HTTPS 트래픽은 Vercel로 직접 보내게 하는 설정이다.
- HTTPS 인증서: 브라우저가 안전한 연결인지 확인할 때 쓰는 인증서다. 이 프로젝트의 프론트 인증서는 Vercel이 자동 발급한다.
- 1016 Origin DNS Error: Cloudflare가 원본 서버 DNS를 해석하지 못할 때 나오는 오류다.
