...
3. `02-architecture-and-rag.md`
   - Spring Boot, MySQL, Python RAG, LangGraph, Gemma, GPT API 후보가 어떻게 연결되는지 본다.
...
- `10대 추천`, `50대 추천`처럼 조건이 있는 질문은 PDF의 가입대상 정보를 보고 맞지 않는 상품을 제외한다.
- 답변근거 API로 검색 score, snippet, PDF 다운로드를 확인할 수 있다.
- 프론트는 Vercel에 정적 배포하고, API는 Cloudflare Tunnel로 개인 Mac의 Spring Boot에 연결한다.
- 대표 질문 18개 기준으로 인사, 무의미 입력, 종료 인사, 상품 설명, 추천, 가입대상, 서류 질문을 회귀 검증한다.

## 지금 새로 잡은 목표

현재 가장 중요한 목표는 답변이 템플릿처럼 보이지 않게 만드는 것이다.  
기본 대화는 GPT처럼 자연스럽게 받아야 하고, 금융상품 정보가 필요한 경우에만 RAG 검색을 적극적으로 사용해야 한다.
...
사용자 입력
  ...
  -> 상품 설명, 추천, 가입대상, 금리, 서류 질문이면 RAG 검색
...
1. 답변 생성 목표를 `자연스러운 대화 + 근거 기반 답변`으로 고정한다.
2. 현재 로컬 Gemma 경로의 프롬프트와 fallback 답변에서 템플릿 느낌을 계속 줄인다.
3. 대표 질문을 더 늘린다. 현재 18개에서 실제 시연 질문 중심으로 20~30개까지 확장한다.
4. GPT API 연결 여부를 결정한다. 필요하면 `RAG_GENERATION_MODE=openai` 같은 별도 생성 모드로 붙인다.
5. 답변근거를 보고 검색이 빗나간 사례를 모은다.
6. 좋은 답변과 나쁜 답변을 피드백으로 모아 LoRA/QLoRA 학습 데이터 후보를 만든다.
7. LoRA/QLoRA는 상품 지식을 외우게 하는 것이 아니라, RAG 근거를 자연스럽고 안전하게 답하는 습관을 학습시키는 용도로 진행한다.

지금은 MSA, 관리자 페이지, OCR, 벡터 DB, 복잡한 추천 알고리즘을 한꺼번에 붙이지 않는다. 이 프로젝트의 강점은 작은 구조 안에서 Spring MVC, MySQL, Python RAG, LLM 생성, 피드백, 배포 흐름을 끝까지 설명할 수 있다는 점이다.
...
- GPT API: OpenAI의 대화형 모델을 API로 호출하는 방식이다. 이 프로젝트에서는 자연스러운 답변 조립 후보로 본다.
