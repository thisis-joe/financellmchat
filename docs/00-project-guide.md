...
2. 현재 로컬 Gemma 경로의 프롬프트와 fallback 답변에서 템플릿 느낌을 줄인다.
3. GPT API 연결 여부를 결정한다. 필요하면 `RAG_GENERATION_MODE=openai` 같은 별도 생성 모드로 붙인다.
4. 대표 질문을 늘린다. 인사, 무의미 입력, 나이 조건, 군인, 고령층, 상품 설명, 상품 추천을 모두 포함한다.
